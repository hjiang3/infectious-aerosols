...
const ASHRAE1 = ({ selectedSubcategory, floorArea, 
      ...
      new Array(yLabels.length)
        ...
        .map((_, y) =>
          new Array(xLabels.length).fill(0).map((_, x) => {
            ...
            return (1 - Math.exp((-9.7 * 19 / (810 * 10) * 8) / newNADR)) * 100;
          })
        );
  ...
  }, [d3Container.current, userValue, airCleaner, uvgi, selectedSubcategory, floorArea,
    height, occupantNumber, occupiedPeriod, expiratoryActivity, physicalActivity,
    virusType, immunityProportion, infectorStatus, casesPerDay, infectiousPeriod, unreportedCases, infectorNumber,
    supplyAir, outdoorAir, merv, hvacUV, hvacTreatment,
    roomTreatment, roomUV, roomAC, roomTreatmentQ, roomUVQ, roomACQ, maskInfector, maskSus ]);
  ...
  <br />
    ...
    <>
      ...
      >
        Total CADR {hoveredNADR}/h&emsp;Absolute Risk: {hoveredAR}%&emsp;Relative Risk: {hoveredRR}<br/>OA: {hoveredOA}&emsp;Filter: {hoveredFilter}%
      </span>
    </>
  )}
</div>
