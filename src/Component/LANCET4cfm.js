...
const LANCET4cfm = ({ selectedSubcategory, floorArea, 
  ...
  immunityProportion, infectorNumber,
  supplyAir, outdoorAir, merv, filter, hvacUV,
  roomUV, roomAC, roomUVQ, roomACQ, maskInfector, maskSus,
  secondQuanta, secondBreath}) => {
...
const [hoveredFilter, setHoveredFilter] = useState(defaultFilter);
  ...
  useEffect(() => {
    ...
    if (d3Container.current) {
      ...
      const newF1 = (occupantNumber - infectorNumber) * (1 - immunityProportion / 100) * secondQuanta * secondBreath * 0.58857777021102 * occupiedPeriod / 60 * (1 - maskInfector / 100) * (1 - maskSus / 100);
      const newF2 = (occupantNumber - infectorNumber) * (1 - immunityProportion / 100)* secondQuanta * secondBreath * 0.58857777021102 * occupiedPeriod / 60 * (1 - maskInfector / 100) * (1 - maskSus / 100);
    ...
    height, occupantNumber, occupiedPeriod, immunityProportion, infectorNumber,
    supplyAir, outdoorAir, merv, filter, hvacUV,
    roomUV, roomAC, roomUVQ, roomACQ, maskInfector, maskSus,
    secondQuanta, secondBreath]);
  ...
  <br />
    ...
    <>
      ...
      >
                Total NADR: {hoveredNADR} cfm&emsp;Target NADR: {((occupantNumber - infectorNumber) * (1 - immunityProportion / 100) * secondQuanta * secondBreath * 0.58857777021102 * occupiedPeriod / 60 * (1 - maskInfector / 100) * (1 - maskSus / 100)).toFixed(1)} cfm<br/> OA: {hoveredOA} cfm&emsp;RA: {(supplyAir-hoveredOA).toFixed(1)} cfm ({((supplyAir - hoveredOA) / supplyAir * 100).toFixed(1)}%)&emsp;Filter: {hoveredFilter}%
        </span>
        <br/>
