...
const HeatmapD3_2 = ({ baseline, occupantNumber, floorArea, height }) => {
    ...
    new Array(yLabels.length)
      ...
      .map((_, y) =>
        new Array(xLabels.length).fill(0).map((_, x) => {
          ...
          return (1 - Math.exp((-9.7 * 19 / (parseFloat(floorArea) * parseFloat(height)) * 8) / NADR)) * 100;
        })
      );
  ...
  }, [d3Container.current, userValue, baseline, occupantNumber, floorArea, height]);
