...
const [hoveredFilter, setHoveredFilter] = useState(defaultFilter);
  ...
  useEffect(() => {
    ...
    if (d3Container.current) {
      ...
      const LANCET_occ = defaultNADR/occupantNumber;
  ...
  <br />
    ...
    <>
      ...
      >
         Total NADR: {hoveredNADR} cfm&emsp;Good: {(21 * occupantNumber).toFixed(0)} cfm&emsp;Better: {(30 * occupantNumber).toFixed(0)} cfm<br/> OA: {hoveredOA} cfm&emsp;RA: {(supplyAir-hoveredOA).toFixed(1)} cfm ({((supplyAir - hoveredOA) / supplyAir * 100).toFixed(1)}%)&emsp;Filter: {hoveredFilter}%
        </span>
        <br/>
    </>
  )}
