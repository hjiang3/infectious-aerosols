...
const ASHRAE1cfm = ({  selectedSubcategory, floorArea, 
  ...
  roomTreatment, roomUV, roomAC, roomTreatmentQ, roomUVQ, roomACQ, maskInfector, maskSus, ASHRAE, 
...
const AR = () => {
  ...
  <br />
    ...
    <>
      ...
      >
        Total ECA {hoveredNADR} cfm&emsp;Individual risk: {hoveredIR}%&emsp;Absolute risk: {hoveredAR}%<br/> OA: {hoveredOA} cfm&emsp;RA: {(supplyAir-hoveredOA).toFixed(1)} cfm ({((supplyAir - hoveredOA) / supplyAir * 100).toFixed(1)}%)&emsp;Filter: {hoveredFilter}%
        </span>
        <br/>
