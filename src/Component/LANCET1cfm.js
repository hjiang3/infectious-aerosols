...
const LANCET1cfm = ({ selectedSubcategory, floorArea, 
  ...
  roomTreatment, roomUV, roomAC, roomTreatmentQ, roomUVQ, roomACQ}) => {
...
const [hoveredFilter, setHoveredFilter] = useState(defaultFilter);
  ...
  <br />
    ...
    <>
      ...
      >
        Total NADR: {hoveredNADR} cfm&emsp;Good: {(4 / 60 * floorArea * height).toFixed(0)} cfm&emsp;Better: {(6 / 60 * floorArea * height).toFixed(0)} cfm<br/> OA: {hoveredOA} cfm&emsp;RA: {(supplyAir-hoveredOA).toFixed(1)} cfm ({((supplyAir - hoveredOA) / supplyAir * 100).toFixed(1)}%)&emsp;Filter: {hoveredFilter}%
        </span>
        <br/>
